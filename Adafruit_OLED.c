...
void writeCommand(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA1_BASE, 0x10, 0x0);       // Enable the SPI CS
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0x0);        // Set the DC pin low

    SPIDataPut(GSPI_BASE, c);                   // send the data
    SPIDataGet(GSPI_BASE,&ulDummy);             // Accept garbage data
    GPIOPinWrite(GPIOA1_BASE, 0x10, 0x10);      // Disable the SPI CS
...
void writeData(unsigned char c) {
    ...
    SPICSEnable(GSPI_BASE);
    GPIOPinWrite(GPIOA1_BASE, 0x10, 0x0);       // Enable the SPI CS
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0x80);        // Set the DC pin high for data
